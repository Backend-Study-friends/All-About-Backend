...
또한 이렇게 변수의 앞에 `volatile` 키워드를 선언해주면 가시성 문제를 해결할 수 있다.

*참고) 가시성 문제 : 하나의 스레드에서 공유 자원을 변경했을 때 바로 메인 메모리에 값이 써지는 것이 아닌 캐시에 값이 저장되는 문제. 이렇게 캐시에 저장된 값이 언제 메인 메모리에 쓰여질지 알 수 없다.*

![img_4.png](img_4.png)

Thread 1에서 counter 값을 0으로 읽고 값을 +1하여 증가시키는 연산을 진행한다.
그리고 Thread1의 값이 메인메모리에 쓰여지기 전에 Thread2에서 counter의 값을 가지고 +1하여 증가시키는 연산을 진행한다고 하자.
두개의 스레드 모두 +1의 연산을 했으니 당연히 2가 나와야 하지만 두 스레드 모두 0의 값을 가지고 연산을 했으니 값은 1이 나올 것이다.

즉, volatile은 멀티스레드 환경에서 오직 한 개의 스레드에서 쓰기/읽기 작업을 할 때, 그리고 다른 스레드에서는 읽기 작업만 할때 안정성을 보장한다.
위의 예제처럼 멀티스레드 환경에서 여러 스레드가 읽기/쓰기 작업을 할 때에는 `synchronized` 키워드로 값의 원자성을 보장해줘야 한다.

(또한, volatile은 메인 메모리에 직접 값을 r/w 하므로 CPU Cache에 접근하는 것보다 많은 비용이 발생하게 된다는 단점이 있다.)
...
### 동기화 문제...
위의 방법을 써봐도 교착 상태/속도 저하의 문제를 해결하려면 더 좋은 방법이 필요하다. 그 방법들(알고리즘들)까지 소개하기에는 `프로세스/스레드`를 설명하는 이 글의 본분에서 벗어나 너무 길어지기 때문에, 별도의 스터디 주제로 만들 것이다.
